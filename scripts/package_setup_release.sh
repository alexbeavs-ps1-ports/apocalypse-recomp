...
#
# Writes: dist/apocalypse-<VERSION>-<artifact-tag>.zip
set -euo pipefail
...
if [[ -d "${ROOT}/launcher_assets" ]] && find "${ROOT}/launcher_assets" -type f -print -quit | grep -q .; then
...
exec bash "${PACKAGER}" \
  ...
  --zip-prefix apocalypse \
  --exe-name Apocalypse_Recompiled \
  --display-name "Apocalypse Recompiled" \
  --recompiler-build "${RECOMPILER_BUILD}" \
  --version-env RELEASE_VERSION \
  --runtime-dir mods \
  --disc-hint "Apocalypse (Europe).cue" \
